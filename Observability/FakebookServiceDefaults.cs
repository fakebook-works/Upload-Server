...
internal static class FakebookServiceDefaults
{
    public static IServiceCollection AddFakebookServiceDefaults(this IServiceCollection services, IConfiguration configuration, string serviceName)
    {
        ...
        telemetry.WithMetrics(metrics => { metrics.AddAspNetCoreInstrumentation().AddHttpClientInstrumentation().AddRuntimeInstrumentation().AddMeter("Fakebook.Upload.MediaLifecycle"); if (endpoint is not null) metrics.AddOtlpExporter(options => options.Endpoint = endpoint); });
